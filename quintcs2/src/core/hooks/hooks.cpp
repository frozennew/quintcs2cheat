...
void c_hooks::init( void ) {
	...
	{ // client.dll
		...
		this->m_client.m_anti_tamper.hook(&c_client_hooks::anti_tamper, g_modules->m_client.find(xx("40 53 41 57 48 83 EC ? 48 89 74 24 ? 48 8B F1")).as<void*>());
		...
		this->m_client.m_level_init.hook(&c_client_hooks::level_init, g_modules->m_client.find(xx("48 89 74 24 ? 57 48 83 EC 30 48 8B 0D ? ? ? ? 48 8B FA")).as<void*>());
